...
use kernelflow_core::{
    hash::hash_json, Dag, KernelError, KernelEvent, KernelResult, NodeContext, NodeInput,
    NodeOutcome, WorkflowNode,
};

use crate::{
    actor::NodeActor,
    resilience::{ResilienceConfig, ResilientExecutor},
};
...
impl Scheduler {
    ...
    pub async fn run<P, F>(&self, dag: &Dag<P>, node_factory: F) -> KernelResult<String>
    where
        P: Send + Sync + 'static,
        F: Fn(&str, &P) -> Arc<dyn WorkflowNode>,
    {
        ...
        for (id, payload) in &dag.nodes {
            actors.insert(
                id.clone(),
                NodeActor::spawn(node_factory(id, payload), exec.clone()),
            );
        }
        ...
        while !ready.is_empty() {
            ...
            for nid in ready.drain(..) {
                ...
                let _ = ev.send(KernelEvent::NodeStarted {
                    workflow_id: dag.id,
                    node_id: nid.clone(),
                    attempt: 1,
                });
                ...
                match res {
                    Ok(out) => {
                        let outcome = NodeOutcome::Success {
                            output_hash: hash_json(&out.value),
                        ...
                        for e in dag.children(&nid) {
                            ...
                            if !e.condition.evaluate(&parent_out) {
                                continue;
                            }
                            let d = indeg.get_mut(&e.to).unwrap();
                            *d -= 1;
                            if *d == 0 {
                                ready.push(e.to.clone());
                            }
                        }
                    }
                    Err(err) => {
        ...
        if outputs.len() != dag.nodes.len() {
            return Err(KernelError::InvalidInput(
                "not all nodes were reached".into(),
            ));
        }
