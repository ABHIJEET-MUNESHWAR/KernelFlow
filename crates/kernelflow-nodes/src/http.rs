...
struct Args { url: String, allowlist: Option<Vec<String>> }

pub struct HttpFetchNode { url: String, allowlist: Vec<String>, timeout: Duration }

impl HttpFetchNode {
    pub fn new(url: impl Into<String>, allowlist: Vec<String>) -> Self {
        Self { url: url.into(), allowlist, timeout: Duration::from_secs(10) }
    }
    fn allowed(&self, url: &str) -> bool {
        if self.allowlist.is_empty() { return true; }
...
impl WorkflowNode for HttpFetchNode {
    fn kind(&self) -> &'static str { "http_fetch" }
    async fn execute(&self, _ctx: &NodeContext, _input: NodeInput) -> KernelResult<NodeOutput> {
        if !self.allowed(&self.url) {
            return Err(KernelError::InvalidInput(format!("host not in allowlist: {}", self.url)));
        }
        ...
        Ok(NodeOutput { value, gas_used: 50 })
    }
}
    ...
    r.register(
        NodeDescriptor {
            ...
            requirements: ResourceRequirements {
                capabilities: vec![Capability::HttpEgress], ..Default::default()
            },
        },
        Arc::new(|args| {
            let a: Args = serde_json::from_value(args.clone())
                .map_err(|e| KernelError::InvalidInput(e.to_string()))?;
            Ok(Arc::new(HttpFetchNode::new(a.url, a.allowlist.unwrap_or_default()))
