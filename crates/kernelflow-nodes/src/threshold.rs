...
use kernelflow_core::{
    KernelError, KernelResult, NodeContext, NodeInput, NodeOutput, ResourceRequirements, WorkflowNode,
};
...
struct Args { threshold: f64, op: Op }

#[derive(Debug, Clone, Copy, Deserialize)]
#[serde(rename_all = "lowercase")]
enum Op { Gt, Gte, Lt, Lte, Eq }

pub struct ThresholdNode { threshold: f64, op: Op }

impl ThresholdNode {
    pub fn gt(threshold: f64) -> Self { Self { threshold, op: Op::Gt } }
}

#[async_trait]
impl WorkflowNode for ThresholdNode {
    fn kind(&self) -> &'static str { "threshold" }
    async fn execute(&self, _ctx: &NodeContext, input: NodeInput) -> KernelResult<NodeOutput> {
        ...
        let n: f64 = input.parents.values().next()
            .and_then(|v| v.as_f64())
        ...
        Ok(NodeOutput { value: serde_json::json!({ "ok": ok, "value": n }), gas_used: 1 })
    }
}
...
pub(crate) fn register(r: &mut NodeRegistry) {
    r.register(
        ...
        Arc::new(|args| {
            ...
            Ok(Arc::new(ThresholdNode { threshold: a.threshold, op: a.op }) as Arc<dyn WorkflowNode>)
        }),
    );
}
...
mod tests {
    ...
    async fn gt_fires() {
        ...
        let out = n.execute(&NodeContext::default(), NodeInput { parents, ..Default::default() }).await.unwrap();
