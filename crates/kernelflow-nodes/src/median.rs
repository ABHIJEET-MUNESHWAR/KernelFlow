...
impl WorkflowNode for MedianNode {
    fn kind(&self) -> &'static str { "median" }
    async fn execute(&self, _ctx: &NodeContext, input: NodeInput) -> KernelResult<NodeOutput> {
        let mut nums: Vec<f64> = input.parents.values()
            .filter_map(|v| v.as_f64().or_else(|| v.get("value").and_then(|x| x.as_f64())))
            .collect();
        if nums.is_empty() {
            return Err(KernelError::InvalidInput("no numeric parents for median".into()));
        }
        nums.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
        let n = nums.len();
        let m = if n % 2 == 1 { nums[n / 2] } else { (nums[n / 2 - 1] + nums[n / 2]) / 2.0 };
        Ok(NodeOutput { value: serde_json::json!(m), gas_used: 3 })
    }
}
...
mod tests {
    ...
    async fn median_of_three() {
        ...
        let input = NodeInput { parents, ..Default::default() };
        let out = MedianNode.execute(&NodeContext::default(), input).await.unwrap();
