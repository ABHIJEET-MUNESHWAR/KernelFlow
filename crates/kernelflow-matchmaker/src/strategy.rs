...
pub trait Matchmaker: Send + Sync {
    async fn select(&self, registry: &OperatorRegistry, req: &ResourceRequirements) -> KernelResult<OperatorId>;
}
...
impl Matchmaker for ReputationMatchmaker {
    async fn select(&self, registry: &OperatorRegistry, req: &ResourceRequirements) -> KernelResult<OperatorId> {
        ...
        if candidates.is_empty() {
            return Err(KernelError::Network("no operator can satisfy requirements".into()));
        }
        // Sort descending by reputation, then random tie-break inside top group.
        candidates.sort_by(|a, b| b.reputation.partial_cmp(&a.reputation).unwrap_or(std::cmp::Ordering::Equal));
        let top_score = candidates[0].reputation;
        let top: Vec<_> = candidates.iter().filter(|o| (o.reputation - top_score).abs() < 1e-9).collect();
        let chosen = top.choose(&mut rand::thread_rng())
...
mod tests {
    ...
    fn cap(caps: Vec<Capability>, vram: u32) -> ResourceCapacity {
        ResourceCapacity { cpu_cores: 8, ram_mb: 32_000, gpu_vram_mb: vram, capabilities: caps }
    }
    ...
    async fn picks_capability_match_with_highest_reputation() {
        ...
        a.reputation = 0.2;
        let mut b = Operator::new("b", cap(vec![Capability::GpuProver { vram_mb: 24_000 }], 24_000));
        b.reputation = 0.9;
        let mut c = Operator::new("c", cap(vec![Capability::GpuProver { vram_mb: 24_000 }], 24_000));
        c.reputation = 0.4;
    ...
    async fn reservation_blocks_doublebooking() {
        let reg = OperatorRegistry::new();
        reg.register(Operator::new("solo", cap(vec![Capability::GenericCpu], 0))).await;
        let req = ResourceRequirements { cpu_cores: 8, capabilities: vec![Capability::GenericCpu], ..Default::default() };
        reg.reserve("solo", &req).await.unwrap();
        // 2nd reservation can't fit.
        assert!(matches!(reg.reserve("solo", &req).await, Err(KernelError::RateLimited)));
